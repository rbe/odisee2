...
package eu.artofcoding.odisee.eu.artofcoding.odisee;
...
import eu.artofcoding.odisee.server.*;
...
public class TextDocumentShowcase {
    ...
    public TextDocumentShowcase(OfficeConnection officeConnection) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Runnable r = new Runnable() {
            ...
            public void run() {
                try {
                    // Setup OfficeConnection
                    OfficeConnection officeConnection = factory.fetchConnection(false);
                    // Load template and process it
                    TextDocumentShowcase showcase = new TextDocumentShowcase(officeConnection);
                    File template = new File(TEMPLATE_PATH, "TextTemplateWithUserFields.ott");
                    OfficeDocument officeDocument = showcase.templateExample(template);
                    // Save and close
                    officeConnection.saveAndCloseDocument(officeDocument, new URL("file:///usr/local/odisee/var/Hello-" + Thread.currentThread().getName() + ".pdf"), true);
                    // Reposit connection
                    factory.repositConnection(officeConnection);
     ...
     * The file contains a number of User text fields (Variables - User) and a bookmark which we use to fill in various values.
     */
    public OfficeDocument templateExample(File sourceFile) throws java.lang.Exception {
        // Load template with User fields and bookmark
        OfficeDocument officeDocument = officeConnection.createDocument(sourceFile);
        XComponent xTemplateComponent = officeDocument.getXComponent();
        // Get XTextFieldsSupplier, XBookmarksSupplier interfaces
        XTextFieldsSupplier xTextFieldsSupplier = queryInterface(XTextFieldsSupplier.class, xTemplateComponent);
        XBookmarksSupplier xBookmarksSupplier = queryInterface(XBookmarksSupplier.class, xTemplateComponent);
        ...
        while (keys.hasMoreElements()) {
            ...
            XPropertySet xPropertySet = queryInterface(XPropertySet.class, fieldMaster);
        ...
        // We need its XTextRange which is available from getAnchor(), so query for XTextContent
        XTextContent xBookmarkContent = queryInterface(XTextContent.class, bookmark);
     ...
     */
    public void viewCursorExample() throws java.lang.Exception {
        XDesktop xDesktop = officeConnection.getXDesktop();
        // retrieve the current component and access the controller
        XComponent xCurrentComponent = xDesktop.getCurrentComponent();
        XModel xModel = (XModel) queryInterface(XModel.class, xCurrentComponent);
        XController xController = xModel.getCurrentController();
        // the controller gives us the TextViewCursor
        XTextViewCursorSupplier xViewCursorSupplier = (XTextViewCursorSupplier) queryInterface(XTextViewCursorSupplier.class, xController);
        XTextViewCursor xViewCursor = xViewCursorSupplier.getViewCursor();
        // query its XPropertySet interface, we want to set character and paragraph
        // properties
        XPropertySet xCursorPropertySet = (XPropertySet) queryInterface(XPropertySet.class, xViewCursor);
        ...
        XPageCursor xPageCursor = (XPageCursor) queryInterface(XPageCursor.class, xViewCursor);
        ...
        XParagraphCursor xParagraphCursor = (XParagraphCursor) queryInterface(XParagraphCursor.class, xModelCursor);
     ...
     * developer's manual
     */
    public void editingExample() throws java.lang.Exception {
        // Create empty swriter document
        OfficeDocument officeDocument = officeConnection.createDocument(OfficeDocumentType.TEXT);
        XComponent xEmptyWriterComponent = officeDocument.getXComponent();
        // Query its XTextDocument interface to get the text
        mxDoc = queryInterface(XTextDocument.class, xEmptyWriterComponent);
        // Get a reference to the body text of the document
        mxDocText = mxDoc.getText();
        // Get a reference to the document's property set. This contains document information like the current word count
        XPropertySet mxDocProps = queryInterface(XPropertySet.class, mxDoc);
        // Simple text insertion example
        bodyTextExample();
        // Example using text ranges to insert strings at the beginning or end of a text range
        textRangeExample();
        // Create a document cursor and remember it, it will be used in most of the following examples
        mxDocCursor = mxDocText.createTextCursor();
        // Demonstrate some of the different cursor types (word, sentence)
        textCursorExample();
        // Access the text document's multi service factory, which we will need for most of the following examples
        mxDocFactory = queryInterface(XMultiServiceFactory.class, mxDoc);
        // Examples of text fields, dependant text fields and field masters
        textFieldExample();
        // Example of using an XEnumerationAccess to iterate over paragraphs and set properties of each paragraph as we do so
        paragraphExample();
        // Example of creating and manipulating a text frame
        textFrameExample();
        // Example of creating and manipulating a text table, text table rows and text table cells get a new random generator
        textTableExample();
        // Example of creating, inserting and manipulating text sections, as well as an example of how to refresh the document
        textSectionExample();
        // Example of creating a text section over a block of text and formatting the text section into columns,
        // as well as how to insert an empty paragraph using the XRelativeTextContentInsert
        textColumnsExample();
        // Example of creating the NumberingRules service and adjusting NumberingTypes and NumberingLevels
        numberingExample();
        // Example of how to use the XStyleFamiliesSupplier interface of the document
        // and how to create, insert and apply styles
        stylesExample();
        indexExample();
        // Example of how to create and manipulate reference marks and GetReference text fields
        referenceExample();
        // Example of how to create and insert Footnotes and how to use the XFootnotesSupplier interface of the document
        footnoteExample();
        // This method demonstrates how to create shapes from the document factory
        // and how to access the draw page of the document using the XDrawPageSupplier interface
        drawPageExample();
        ...
        mxFactory = queryInterface(XMultiServiceFactory.class, officeConnection.getXRemoteServiceManager());
        autoTextExample();
        // Close
     ...
     */
    public void storeDocComponent(XComponent xDoc, String storeUrl) throws java.lang.Exception {
        XStorable xStorable = (XStorable) queryInterface(XStorable.class, xDoc);
    ...
    public void printDocComponent(XComponent xDoc, String aPrinterName) throws java.lang.Exception {
        XPrintable xPrintable = (XPrintable) queryInterface(XPrintable.class, xDoc);
    ...
    public void storePrintExample(String sOutputDir, String aPrinterName) throws java.lang.Exception {
        /*
        ...
        */
        XComponentLoader xComponentLoader = officeConnection.getXComponentLoader();
        ...
        if (xDoc != null) {
            ...
            printDocComponent(xDoc, aPrinterName);
        }
    }
     ...
     */
    public void textCursorExample() {
        try {
            // First, get the XSentenceCursor interface of our text cursor
            XSentenceCursor xSentenceCursor = (XSentenceCursor) queryInterface(XSentenceCursor.class, mxDocCursor);
            ...
            XWordCursor xWordCursor = (XWordCursor) queryInterface(XWordCursor.class, mxDocCursor);
            ...
            XPropertySet xCursorProps = (XPropertySet) queryInterface(XPropertySet.class, mxDocCursor);
    ...
    public void textFieldExample() {
        try {
            ...
            XTextField xDateField = (XTextField) queryInterface(XTextField.class, mxDocFactory.createInstance("com.sun.star.text.TextField.DateTime"));
            ...
            XDependentTextField xUserField = (XDependentTextField) queryInterface(XDependentTextField.class, mxDocFactory.createInstance("com.sun.star.text.TextField.User"));
            // Create a fieldmaster for our newly created User Text field, and
            // access it's XPropertySet interface
            XPropertySet xMasterPropSet = (XPropertySet) queryInterface(XPropertySet.class, mxDocFactory.createInstance("com.sun.star.text.fieldmaster.User"));
    ...
    public void paragraphExample() {
        try {
            ...
            XEnumerationAccess xParaAccess = (XEnumerationAccess) queryInterface(XEnumerationAccess.class, mxDocText);
            ...
            while (xParaEnum.hasMoreElements()) {
                ...
                XServiceInfo xInfo = (XServiceInfo) queryInterface(XServiceInfo.class, xParaEnum.nextElement());
                if (!xInfo.supportsService("com.sun.star.text.TextTable")) {
                    // Access the paragraph's property set...the properties in this
                    // property set are listed in:
                    // com.sun.star.style.ParagraphProperties
                    XPropertySet xSet = (XPropertySet) queryInterface(XPropertySet.class, xInfo);
    ...
    public static void insertIntoCell(String sCellName, String sText, XTextTable xTable) {
        ...
        XText xCellText = (XText) queryInterface(XText.class, xTable.getCellByName(sCellName));
        // create a text cursor from the cells XText interface
        XTextCursor xCellCursor = xCellText.createTextCursor();
        // Get the property set of the cell's TextCursor
        XPropertySet xCellCursorProps = (XPropertySet) queryInterface(XPropertySet.class, xCellCursor);
        try {
     ...
     */
    public void textTableExample() {
        try {
            // Create a new table from the document's factory
            XTextTable xTable = (XTextTable) queryInterface(XTextTable.class, mxDocFactory.createInstance("com.sun.star.text.TextTable"));
            ...
            XPropertySet xRow = (XPropertySet) queryInterface(XPropertySet.class, xRows.getByIndex(0));
            ...
            XPropertySet xTableProps = (XPropertySet) queryInterface(XPropertySet.class, xTable);
            ...
            // Insert random numbers into the first this three cells of each remaining row
            ...
            // Set the last cell in each row to be a formula that calculates the sum of the first three cells
     ...
     */
    public void textFrameExample() {
        try {
            // Use the document's factory to create a new text frame and
            // immediately access it's XTextFrame interface
            XTextFrame xFrame = (XTextFrame) queryInterface(XTextFrame.class, mxDocFactory.createInstance("com.sun.star.text.TextFrame"));
            // Access the XShape interface of the TextFrame
            XShape xShape = (XShape) queryInterface(XShape.class, xFrame);
            // Access the XPropertySet interface of the TextFrame
            XPropertySet xFrameProps = (XPropertySet) queryInterface(XPropertySet.class, xFrame);
    ...
    public void autoTextExample() {
        try {
            ...
            XParagraphCursor xParaCursor = (XParagraphCursor) queryInterface(XParagraphCursor.class, mxDocCursor);
            xParaCursor.gotoPreviousParagraph(false);
            // Get an XNameAccess interface to all auto text groups from the document factory
            XNameAccess xContainer = (XNameAccess) queryInterface(XNameAccess.class, mxFactory.createInstance("com.sun.star.text.AutoTextContainer"));
            ...
            if (aGroupNames.length > 0) {
                ...
                for (int i = 0; i < aGroupNames.length; i++) {
                    ...
                    XAutoTextGroup xGroup = (XAutoTextGroup) queryInterface(XAutoTextGroup.class, xContainer.getByName(aGroupNames[i]));
                    ...
                    if (aBlockNames.length > 0) {
                        ...
                        for (int j = 0; j < aBlockNames.length; j++) {
                            // Get the XText interface of the current cell
                            xCellText = (XText) queryInterface(XText.class, xTable.getCellByName(xTableCursor.getRangeName()));
                ...
                do {
                    ...
                    xGroup = (XAutoTextGroup) queryInterface(XAutoTextGroup.class, xContainer.getByName(aGroupNames[nRandom]));
                ...
                if (aBlockNames.length > 0) {
                    ...
                    XAutoTextEntry xEntry = (XAutoTextEntry) queryInterface(XAutoTextEntry.class, xGroup.getByName(aBlockNames[nRandom]));
                    ...
                    XNamed xGroupNamed = (XNamed) queryInterface(XNamed.class, xGroup);
    ...
    public void indexExample() {
        try {
            ...
            XPropertySet xEntry = (XPropertySet) queryInterface(XPropertySet.class, mxDocFactory.createInstance("com.sun.star.text.ContentIndexMark"));
            ...
            XPropertySet xIndex = (XPropertySet) queryInterface(XPropertySet.class, mxDocFactory.createInstance("com.sun.star.text.ContentIndex"));
            ...
            // IndexMark
            XTextContent xIndexContent = (XTextContent) queryInterface(XTextContent.class, xIndex);
            XTextContent xEntryContent = (XTextContent) queryInterface(XTextContent.class, xEntry);
            ...
            XDocumentIndex xDocIndex = (XDocumentIndex) queryInterface(XDocumentIndex.class, xIndex);
    ...
    public void referenceExample() {
        try {
            ...
            XNamed xRefMark = (XNamed) queryInterface(XNamed.class, mxDocFactory.createInstance("com.sun.star.text.ReferenceMark"));
            // Set the name to TableHeader
            xRefMark.setName("TableHeader");
            // Get the TextTablesSupplier interface of the document
            XTextTablesSupplier xTableSupplier = (XTextTablesSupplier) queryInterface(XTextTablesSupplier.class, mxDoc);
            // Get an XIndexAccess of TextTables
            XIndexAccess xTables = (XIndexAccess) queryInterface(XIndexAccess.class, xTableSupplier.getTextTables());
            // We've only inserted one table, so get the first one from index zero
            XTextTable xTable = (XTextTable) queryInterface(XTextTable.class, xTables.getByIndex(0));
            // Get the first cell from the table
            XText xTableText = (XText) queryInterface(XText.class, xTable.getCellByName("A1"));
            ...
            // insert it
            XTextContent xContent = (XTextContent) queryInterface(XTextContent.class, xRefMark);
            ...
            XPropertySet xFieldProps = (XPropertySet) queryInterface(XPropertySet.class, mxDocFactory.createInstance("com.sun.star.text.TextField.GetReference"));
            // Get the XReferenceMarksSupplier interface of the document
            XReferenceMarksSupplier xRefSupplier = (XReferenceMarksSupplier) queryInterface(XReferenceMarksSupplier.class, mxDoc);
            // Get an XNameAccess which refers to all inserted reference marks
            XNameAccess xMarks = (XNameAccess) queryInterface(XNameAccess.class, xRefSupplier.getReferenceMarks());
            ...
            if (aNames.length > 0) {
                ...
                XTextContent xRefContent = (XTextContent) queryInterface(XTextContent.class, xFieldProps);
                // Go to the end of the document
                mxDocCursor.gotoEnd(false);
                ...
                XRefreshable xRefresh = (XRefreshable) queryInterface(XRefreshable.class, mxDoc);
                xRefresh.refresh();
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    public void footnoteExample() {
        try {
            // Create a new footnote from the document factory and get it's
            // XFootnote interface
            XFootnote xFootnote = (XFootnote) queryInterface(XFootnote.class, mxDocFactory.createInstance("com.sun.star.text.Footnote"));
            // Set the label to 'Numbers'
            xFootnote.setLabel("Numbers");
            // Get the footnotes XTextContent interface so we can...
            XTextContent xContent = (XTextContent) queryInterface(XTextContent.class, xFootnote);
            // ...insert it into the document
            mxDocText.insertTextContent(mxDocCursor, xContent, false);
            // Get the XFootnotesSupplier interface of the document
            XFootnotesSupplier xFootnoteSupplier = (XFootnotesSupplier) queryInterface(XFootnotesSupplier.class, mxDoc);
            // Get an XIndexAccess interface to all footnotes
            XIndexAccess xFootnotes = (XIndexAccess) queryInterface(XIndexAccess.class, xFootnoteSupplier.getFootnotes());
            // Get the XFootnote interface to the first footnote inserted ('Numbers')
            XFootnote xNumbers = (XFootnote) queryInterface(XFootnote.class, xFootnotes.getByIndex(0));
            // Get the XSimpleText interface to the Footnote
            XSimpleText xSimple = (XSimpleText) queryInterface(XSimpleText.class, xNumbers);
            // Create a text cursor for the foot note text
            XTextRange xRange = (XTextRange) queryInterface(XTextRange.class, xSimple.createTextCursor());
    ...
    public void drawPageExample() {
        try {
            ...
            XShape xRect = (XShape) queryInterface(XShape.class, mxDocFactory.createInstance("com.sun.star.drawing.RectangleShape"));
            // Create an EllipseShape using the document factory
            XShape xEllipse = (XShape) queryInterface(XShape.class, mxDocFactory.createInstance("com.sun.star.drawing.EllipseShape"));
            ...
            XPropertySet xRectProps = (XPropertySet) queryInterface(XPropertySet.class, xRect);
            XPropertySet xEllipseProps = (XPropertySet) queryInterface(XPropertySet.class, xEllipse);
            ...
            XDrawPageSupplier xDrawPageSupplier = (XDrawPageSupplier) queryInterface(XDrawPageSupplier.class, mxDoc);
            // Get the XShapes interface of the draw page
            XShapes xShapes = (XShapes) queryInterface(XShapes.class, xDrawPageSupplier.getDrawPage());
            // Add both shapes
            ...
            /*
              ...
              XShapes xGrouper = (XShapes) UnoRuntime.queryInterface(XShapes.class, mxDocFactory.createInstance("com.sun.star.drawing.GroupShape"));
              ...
              XShapeGrouper xShapeGrouper = (XShapeGrouper) UnoRuntime.queryInterface(XShapeGrouper.class, xShapes);
    ...
    public void stylesExample() {
        try {
            ...
            XStyle xStyle = (XStyle) queryInterface(XStyle.class, mxDocFactory.createInstance("com.sun.star.style.ParagraphStyle"));
            // Access the XPropertySet interface of the new style
            XPropertySet xStyleProps = (XPropertySet) queryInterface(XPropertySet.class, xStyle);
            // Give the new style a light blue background
            xStyleProps.setPropertyValue("ParaBackColor", new Integer(13421823));
            // Get the StyleFamiliesSupplier interface of the document
            XStyleFamiliesSupplier xSupplier = (XStyleFamiliesSupplier) queryInterface(XStyleFamiliesSupplier.class, mxDoc);
            // Use the StyleFamiliesSupplier interface to get the XNameAccess
            // interface of the actual style families
            XNameAccess xFamilies = (XNameAccess) queryInterface(XNameAccess.class, xSupplier.getStyleFamilies());
            // Access the 'ParagraphStyles' Family
            XNameContainer xFamily = (XNameContainer) queryInterface(XNameContainer.class, xFamilies.getByName("ParagraphStyles"));
     ...
     */
    public void numberingExample() {
        try {
            // Go to the end of the document
            mxDocCursor.gotoEnd(false);
            // Get the RelativeTextContentInsert interface of the document
            XRelativeTextContentInsert xRelative = (XRelativeTextContentInsert) queryInterface(XRelativeTextContentInsert.class, mxDocText);
            // Use the document's factory to create the NumberingRules service,
            // and get it's XIndexAccess interface
            XIndexAccess xNum = (XIndexAccess) queryInterface(XIndexAccess.class, mxDocFactory.createInstance("com.sun.star.text.NumberingRules"));
            // Also get the NumberingRule's XIndexReplace interface
            XIndexReplace xReplace = (XIndexReplace) queryInterface(XIndexReplace.class, xNum);
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                XTextContent xNewPara = (XTextContent) queryInterface(XTextContent.class, mxDocFactory.createInstance("com.sun.star.text.Paragraph"));
                // Get the XPropertySet interface of the new paragraph and put
                // it in our array
                xParas[i] = (XPropertySet) queryInterface(XPropertySet.class, xNewPara);
                    ...
                    if (aProps[j].Name.equals("NumberingType")) {
                        ...
                        switch (i) {
                            case 0:
                                ...
                                break;
                            case 1:
                                aProps[j].Value = new Short(NumberingType.CHARS_UPPER_LETTER);
                                break;
                            case 2:
                                aProps[j].Value = new Short(NumberingType.ARABIC);
                                break;
                        }
     ...
     */
    public void textSectionExample() {
        try {
            // Go to the end of the document
            mxDocCursor.gotoEnd(false);
            // Insert two paragraph breaks
            ...
            XNamed xChildNamed = (XNamed) queryInterface(XNamed.class, mxDocFactory.createInstance("com.sun.star.text.TextSection"));
            // Set the new sections name to 'Child_Section'
            xChildNamed.setName("Child_Section");
            // Access the Child_Section's XTextContent interface and insert it
            // into the document
            XTextContent xChildSection = (XTextContent) queryInterface(XTextContent.class, xChildNamed);
            ...
            XNamed xParentNamed = (XNamed) queryInterface(XNamed.class, mxDocFactory.createInstance("com.sun.star.text.TextSection"));
            ...
            XTextContent xParentSection = (XTextContent) queryInterface(XTextContent.class, xParentNamed);
            ...
            XNamed xLinkNamed = (XNamed) queryInterface(XNamed.class, mxDocFactory.createInstance("com.sun.star.text.TextSection"));
            ...
            XTextContent xLinkedSection = (XTextContent) queryInterface(XTextContent.class, xLinkNamed);
            ...
            XPropertySet xLinkProps = (XPropertySet) queryInterface(XPropertySet.class, xLinkNamed);
            ...
            XPropertySet xChildProps = (XPropertySet) queryInterface(XPropertySet.class, xChildNamed);
     ...
     */
    public void textColumnsExample() {
        try {
            ...
            XNamed xSectionNamed = (XNamed) queryInterface(XNamed.class, mxDocFactory.createInstance("com.sun.star.text.TextSection"));
            ...
            XTextColumns xColumns = (XTextColumns) queryInterface(XTextColumns.class, mxDocFactory.createInstance("com.sun.star.text.TextColumns"));
            ...
            XPropertySet xSectionProps = (XPropertySet) queryInterface(XPropertySet.class, xSectionNamed);
            ...
            mxFishSection = (XTextContent) queryInterface(XTextContent.class, xSectionNamed);
