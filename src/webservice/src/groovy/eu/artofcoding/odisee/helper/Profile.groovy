...
final class Profile {
    ...
    static time = { String text, Closure closure ->
        if (OdiseePath.ODISEE_PROFILE) {
            ...
            println "${new java.util.Date().format(DATE_FORMAT)} ODI-xxxx: ${Thread.currentThread().name}: BEGIN-${counter} ${text}"
            // Remember start time
            ...
            println "${new java.util.Date().format(DATE_FORMAT)} ODI-xxxx: ${Thread.currentThread().name}: END-${counter} ${text}, ${total}, ${user}, ${sys}, ${wall}, ${wallGap}"
            // Return result
            result
        } else {
            closure()
